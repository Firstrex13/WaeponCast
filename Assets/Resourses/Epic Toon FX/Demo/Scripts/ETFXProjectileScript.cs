...
public class ETFXProjectileScript : MonoBehaviour
{
    ...
    Collider[] bufer = new Collider[16];
    ...
    void FixedUpdate()
    {
        ...
        int count = Physics.OverlapSphereNonAlloc(transform.position, radius, bufer, Mask);

        if (count > 0) // Checks if collision will happen
        {
            // transform.position = hit.point + (hit.normal * collideOffset); // Move projectile to point of collision

           // GameObject impactP = Instantiate(impactParticle, transform.position, Quaternion.FromToRotation(Vector3.up, bufer[0].transform.rotation.eulerAngles)) as GameObject; // Spawns impact effect
            GameObject impactP = Instantiate(impactParticle, transform.position, Quaternion.FromToRotation(Vector3.up, Vector3.right)) as GameObject; // Spawns impact effect
            Debug.Log($"{impactP == null} взрыв {bufer[0].gameObject.name}");
            ...
            Destroy(impactP, 2f); // Removes impact effect after delay
